...
namespace clp
{
	...
	template<>
	size_t CommandLineArgumentParser::GetArgumentValue<size_t>(const std::string &option) const
	{
		return static_cast<size_t>(std::atoi(GetArgumentValue<std::string>(option).c_str()));
	}
